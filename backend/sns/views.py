...
from sns.models import Friend, Good, Group, Message

from rest_framework import viewsets
from rest_framework.views import APIView
from rest_framework.decorators import api_view
from rest_framework.response import Response
from .serializer import FriendSerializer, GroupSerializer, MessageSerializer, UserSerializer
...
# フレンドを返します。
class GetFriendApiView(APIView):
    def get(self, request, *args, **kwargs):
        friend = Friend.objects.filter(id=request.query_params['id'])
        selializer = FriendSerializer(friend)
...
class GetUserApiView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        seliarizer = UserSerializer(user)
        return Response(seliarizer.data)


# 自身が所属するグループを検索します。
class FindMyGroupApiView(APIView):
    def get(self, request, *args, **kwargs):
...
# 指定されたグループに投稿されたメッセージを検索します。
class FindMessageApiView(APIView):
    def get(self, request, *args, **kwargs):
        group = Group.objects.filter(owner=request.query_params['owner']).filter(
            title=request.query_params['title'])
        messages = Message.objects.filter(group__in=group)
...
# メッセージを投稿します。
class PostMessageApiView(APIView):
    def post(self, request, *args, **kwargs):
        param = request.data

        group_owner = User.objects.filter(id=param['owner']).first()
        group_title = Group.objects.filter(id=param['group']).first().title
        if Group.objects.filter(owner=group_owner).filter(title=group_title).first() == None:
            (public_user, param.group) = get_public()
        serializer = MessageSerializer(data=param)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
...
from sns.forms import CreateGroupForm, FriendsForm, GroupCheckForm, GroupSelectForm, PostForm, SearchForm
...
def share(request, share_id):
    # シェアするメッセージの取得
    share = Message.objects.filter(id=share_id).first()
...
def get_your_group_message(owner, glist, find):
    ...
    (public_user, public_group) = get_public()
    # チェックされたGroupの取得
    groups = Group.objects.filter(Q(owner=owner) | Q(
        owner=public_user)).filter(title__in=glist)
    # Groupに含まれるFriendの取得
    ...
    his_friends = Friend.objects.filter(
        user=owner).filter(group__in=his_groups)
    me_groups = []
    ...
    if find == None:
        messages = Message.objects.filter(
            Q(group__in=groups) | Q(group__in=me_groups))[:100]
    else:
        messages = Message.objects.filter(Q(group__in=groups) | Q(
            group__in=me_groups)).filter(content__contains=find)
    return messages
